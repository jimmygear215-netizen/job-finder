...
def analyze_with_ai(resume_text, url_data, url_skills, career_intent, endpoint, api_key="", model=""):
    ...
    url_skills_str = ', '.join(url_skills[:20]) if url_skills else ""

    extra = ""
    if career_intent == "dream":
        extra = " They want to change careers. Identify transferable skills that could apply in different industries."

    prompt = f"""A user is looking for a job and wants to {intent}.{extra}
...
URL INFORMATION:
{urls_str[:1000]}
{"SKILLS FROM ONLINE PROFILES: " + url_skills_str if url_skills_str else ""}
...
def get_next_question(profile, career_intent, qa_history, question_number, url_data, endpoint, api_key="", model=""):
    ...
    url_str = "\n".join(url_data[:5]) if url_data else ""

    extra = ""
    if career_intent == "dream":
        extra = " They're changing careers. Ask about what draws them to the new path, transferable skills they enjoy using, or what they're looking for in a different field."

    prompt = f"""You are a career advisor. The user wants to {intent}.{extra}

THEIR PROFILE: {profile_flat}
{"ONLINE PROFILES: " + url_str if url_str else ""}
...
def suggest_roles(profile, career_intent, qa_history, endpoint, api_key="", model=""):
    ...
    extra = ""
    if career_intent == "dream":
        extra = " They're changing careers. Suggest bridge roles that leverage their transferable skills into a new direction. Include some stretch roles too."

    prompt = f"""You are a career advisor suggesting jobs for a user. They want to {intent}.{extra}
