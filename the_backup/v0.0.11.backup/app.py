...
def analyze_resume(resume_text):
    ...
    for skill, keywords in skill_keywords.items():
        hits = sum(1 for kw in keywords if re.search(r'\b' + re.escape(kw) + r'\b', resume_lower))
        if hits > 0:
            skills_found.append(skill)
            skill_frequencies[skill] = hits
    
    year_match = re.search(r'(\d+)\+?\s*(?:year|yr)s?\s*(?:of\s+)?(?:experience|driving|work|delivery)?', resume_lower)
    if year_match:
...
def combine_analysis_endpoint():
    ...
    resume_frequencies = analysis.get('skill_frequencies', {}) if analysis else {}

    return jsonify({
        'skills_found': sorted(set(all_skills)),
        'resume_skills': resume_skills,
        'skill_frequencies': resume_frequencies,
        'experience_level': experience_level,
...
def follow_up_questions_endpoint():
    ...
    skill_frequencies = data.get('skill_frequencies', {})
    ...
    if career_intent == 'same' and skill_frequencies:
        top_skill = max(skill_frequencies, key=skill_frequencies.get)
        if top_skill in TOPIC_QUESTIONS:
            matched_topics.add(top_skill)
    else:
