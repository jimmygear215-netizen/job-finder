...
def ai_analyze_endpoint():
    ...
    try:
        result = ai_analyzer.analyze_with_ai(resume_text, url_data, career_intent, endpoint, api_key, model)
        return jsonify(result)
    except requests.RequestException as e:
...
def ai_question_endpoint():
    ...
    try:
        result = ai_analyzer.get_next_question(profile, career_intent, qa_history, question_number, endpoint, api_key, model)
        return jsonify(result)
    except requests.RequestException as e:
        return jsonify({'error': f'AI connection failed: {str(e)}', 'fallback': True}), 503
